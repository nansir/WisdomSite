...
/**
 * 照片缩放视图
 * Created by zhuyinan on 2018/4/1.
 */
public class PhotoZoomView extends AppCompatImageView {
    ...
    private RectF mClip;
    private PhotoZoomInfo mFromInfo;
...
//          mScaleCenter.set(detector.getFocusX(), detector.getFocusY());
    ...
    private Runnable mClickRunnable = new Runnable() {
        ...
        public void run() {
            if (mClickListener != null) {
                mClickListener.onClick(PhotoZoomView.this);
            }
        }
    };
    private GestureDetector.OnGestureListener mGestureListener = new GestureDetector.SimpleOnGestureListener() {
        ...
        public void onLongPress(MotionEvent e) {
            if (mLongClick != null) {
                mLongClick.onLongClick(PhotoZoomView.this);
            }
        }
    ...
    public PhotoZoomView(Context context, AttributeSet attrs, int defStyleAttr) {
    ...
    public static PhotoZoomInfo getImageViewInfo(ImageView imgView) {
        int[] p = new int[2];
        ...
        return new PhotoZoomInfo(rect, imgRect, widgetRect, baseRect, screenCenter, 1, 0, imgView.getScaleType());
    }
    ...
    public PhotoZoomInfo getInfo() {
        RectF rect = new RectF();
        int[] p = new int[2];
        getLocation(this, p);
        rect.set(p[0] + mImgRect.left, p[1] + mImgRect.top, p[0] + mImgRect.right, p[1] + mImgRect.bottom);
        return new PhotoZoomInfo(rect, mImgRect, mWidgetRect, mBaseRect, mScreenCenter, mScale, mDegrees, mScaleType);
    }
     ...
     */
    public void animaFrom(PhotoZoomInfo info) {
        if (isInit) {
            reset();

            PhotoZoomInfo mine = getInfo();
    ...
    public void animaTo(PhotoZoomInfo info, Runnable completeCallBack) {
        if (isInit) {
            mTranslate.stop();
